...
For Models 1 to 4, the paper analyzes stochastic small open economy RBC models subject to productivity shocks:
...
The main quantitative comparison in the paper concerns different mechanisms used to induce stationarity in net foreign assets. These mechanisms modify the interest rate or discount factor dynamics while preserving similar short-run business-cycle properties.

The extension developed in this project concerns Model 5, which is solved under perfect foresight rather than perturbation methods. In this case, the computational problem becomes a nonlinear transition-path problem over a finite horizon.

The economy is initialized with a temporary productivity shock, and the entire transition path is solved simultaneously subject to equilibrium conditions and terminal restrictions. The nonlinear system includes equilibrium conditions such as:

$$
c_t + i_t + d_t = y_t + (1+r)d_{t-1}
$$

together with Euler equations and capital accumulation dynamics:
